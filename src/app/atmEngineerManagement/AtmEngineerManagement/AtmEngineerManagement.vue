<template>
  <div :class="$style.atmEngineerManagement">
    <vue-grid>
      <vue-breadcrumb :items="breadCrumbItems"></vue-breadcrumb>

      <vue-grid-row>
        <vue-grid-item fill>
          <vue-headline level="1">AtmEngineerManagement</vue-headline>
        </vue-grid-item>
      </vue-grid-row>
    </vue-grid>
  </div>
...
export default {
  ...
  computed: {
    breadCrumbItems() {
      return [
        ...
        { label: this.$t('common.AtmEngineerManagement' /* AtmEngineerManagement */), href: '/atm-engineer-management' },
      ];
    },
  },
  beforeCreate() {
    registerModule('atmEngineerManagement', AtmEngineerManagementModule);
  },
  prefetch: (options: IPreLoad) => {
     ...
     *
     * return options.store.dispatch('fetchAtmEngineerManagement', '1');
     *
