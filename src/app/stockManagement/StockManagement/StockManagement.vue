<template>
  <div :class="$style.stockManagement">
    <vue-grid>
      <vue-breadcrumb :items="breadCrumbItems"></vue-breadcrumb>

      <vue-grid-row>
        <vue-grid-item fill>
          <vue-headline level="1">StockManagement</vue-headline>
        </vue-grid-item>
      </vue-grid-row>
    </vue-grid>
  </div>
...
export default {
  ...
  computed: {
    breadCrumbItems() {
      return [
        ...
        { label: this.$t('common.StockManagement' /* StockManagement */), href: '/stock-management' },
      ];
    },
  },
  beforeCreate() {
    registerModule('stockManagement', StockManagementModule);
  },
  prefetch: (options: IPreLoad) => {
     ...
     *
     * return options.store.dispatch('fetchStockManagement', '1');
     *
